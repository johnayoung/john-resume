...
tldr:
  - "Default to one agent on coding work, and add an edge only when you can name the constraint that forced it: context pollution, parallelism, or specialization. Risk is not on that list; it earns a review checkpoint, not another worker."
  - "On SWE-bench Verified, every multi-agent architecture underperformed the single-agent baseline, from −2.1% (hybrid) to −14.9% (independent) on the study's 20-instance subsets. That is directional evidence rather than a decisive number, but it pointed uniformly the wrong way for \"more agents means more throughput.\""
...
- Agent = an AI assistant that can take actions on its own, like writing code or running commands, not just answering questions.
...
Downstream of the collision, the PRs behaved exactly as you would predict. They "often conflicted with one-another, at which point they were then abandoned," and Anthropic reads the merge rate the same way:

> "A very low fraction of these PRs were merged, which suggests a lack of coordination."
...
Same models, different arrangements, wildly different behavior. The report's own conclusion is the sentence to pin above the design doc:
...
The closed list as a working rule, where seeing the signal licenses that edge and only that edge:
...
Now apply the rule backward to the thirty agents. They were not isolating context, [the work was not partitioned](/blog/task-decomposition-for-ai-coding-agents/), and no agent held a tool the others lacked. Thirty edges, zero justifications; `mvp-game-loop` is what that looks like in the git log.
...
- **"Reuse the constraints you already tiered"** is derived from Anthropic's closed list of three forcing conditions plus OpenAI's naming of approval surfaces as a cost of splitting. No source maps topology edges onto task-risk tiers.
- **"Draw the human checkpoint as a node, not a stage"** is derived from LangGraph implementing human pauses as in-node interrupts backed by the graph's persistence layer, against the verified absence of any human-review pattern in frontier-lab architecture guidance.
- **"Budget every edge in reviewer minutes"** is derived from OpenAI listing approval surfaces as a cost input and Anthropic's 3-10x token multiplier. Neither source treats reviewer bandwidth as a binding constraint on topology; that step is mine.
