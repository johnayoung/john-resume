...
A non-trivial feature decomposes into a handful of well-sized tasks. The phone-number feature splits into four, one per architectural layer:
...
### Goal
Add a `ValidatePhone` method to `UserService` that validates a
phone number using the internal `validate.PhoneE164` helper,
returns the `*validate.Error` it produces unmodified, and is
unit-tested against valid, invalid, nil, and empty inputs. This
is the service-layer slice of the larger phone-number feature;
the handler and DB layers are out of scope here and live in
separate tasks.

### Architectural Context
- Go 1.22 project, layered architecture:
  `handler → service → repository`.
- Validation lives in the service layer, not the handler. The
  handler does basic null/shape checks; semantic validation
  (E.164 format, length bounds) is the service's responsibility.
- `internal/pkg/validate/` is the project's validation package.
  `validate.PhoneE164(string) error` already exists and returns
  `*validate.Error` on failure. Do not introduce a third-party
  validation library.
- Data access goes through `UserRepository`, generated by sqlc.
  In service-layer tests the repository is mocked via
  `internal/user/mocks/repository_mock.go`.
- Service tests use the standard library `testing` package plus
  the existing `mocks` package — no testify, no ginkgo.

### Relevant Files
- `internal/user/service.go` — Add `ValidatePhone` here.
- `internal/user/service_test.go` — Add tests here.
- `internal/pkg/validate/phone.go` — Read-only reference. Defines
  `PhoneE164` and `validate.Error`.
- `internal/user/mocks/repository_mock.go` — Read-only. Mock used
  in existing service tests.

### Reference Implementation
Mirror `UserService.ValidateEmail` in `internal/user/service.go`
exactly. The new method should match its:
- Function signature shape:
  `func (s *UserService) ValidatePhone(phone *string) error`.
- Nil-handling pattern — a nil pointer means "field not
  provided," return nil; empty string is malformed input and must
  return an error.
- Error wrapping — return the `*validate.Error` from
  `validate.PhoneE164` unmodified. Do not wrap with `fmt.Errorf`.
- Test layout — see `TestUserService_ValidateEmail` in
  `service_test.go` for the table-driven structure to copy.

### Constraints
- Use `validate.PhoneE164`. Do not call a regex or an external
  library directly.
- Do not change `UserRepository` or its mock — validation is pure
  and does not touch the repo.
- Do not double-validate. The handler's null/shape check stays;
  the service owns semantic validation only.
- The error on failure must be a `*validate.Error` so the
  handler's `errors.As(&validate.Error{})` mapping to HTTP 422
  continues to work.

### Non-Goals
- No handler changes (those are Task 4).
- No migration or sqlc changes (Tasks 1 and 2).
- No integration tests — only unit tests against the mocked
  repository.
- No changes to `ValidateEmail` or any other unrelated service
  method.

### Edge Cases
- **Nil vs empty string.** `phone == nil` means the user didn't
  supply a phone (allowed — return nil). `*phone == ""` is
  malformed input and must return a `validate.Error`.
- **Strict E.164.** Numbers must include a leading `+` and
  country code. `1234567890` (no `+`) must fail.
- **Double-validation pitfall.** The handler already verifies
  the JSON field is present and is a string. Do not re-check
  those concerns in the service — only validate semantics.
- **Error type wrapping.** `fmt.Errorf("invalid phone: %w", err)`
  breaks the handler's `errors.As(&validate.Error{})` check.
  Return the `*validate.Error` from `validate.PhoneE164`
  unmodified.

### Acceptance Criteria
1. `UserService` exposes
   `ValidatePhone(phone *string) error`.
2. Returns `nil` when `phone == nil`.
3. Returns a `*validate.Error` (verifiable via `errors.As`)
   when `*phone` is empty or non-E.164.
4. Returns `nil` for a valid E.164 number (e.g.,
   `+14155552671`).
5. `service_test.go` adds at least four cases: valid, invalid
   format, nil, empty string.
6. `go test ./internal/user/... -run TestValidatePhone` passes.
7. No changes to files outside `internal/user/service.go` and
...
1. **Tests first** — Add `TestValidatePhone` table-driven cases
   (all failing initially). Commit.
2. **Implementation** — Add `ValidatePhone` until all tests
   pass. Commit.
This is one task and one PR; do not split further.
````
